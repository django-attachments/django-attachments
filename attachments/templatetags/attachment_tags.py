...
def do_get_attachments(parser, token):
    ...
    error_string = "%r tag must be of format {%% get_attachments for OBJECT as CONTEXT_VARIABLE %%}" % bits[0]
    order_by = None
    if len(bits) == 5:
        try:
        ...
        tag, word_for, content_object, word_as, context_name, word_order_by, order_by = bits
    else:
        raise template.TemplateSyntaxError(error_string)
